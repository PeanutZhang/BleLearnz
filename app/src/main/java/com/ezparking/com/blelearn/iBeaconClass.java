	...
	public static iBeacon fromScanData(BluetoothDevice device, int rssi, byte[] scanData) {
		...
		while (startByte <= 5) {
			if (((int) scanData[startByte + 2] & 0xff) == 0x02 && ((int) scanData[startByte + 3] & 0xff) == 0x15) {
				// 这是 iBeacon
				Log.e("zyh","startByte-----> "+o.hashCode()+"--> true1 "+ startByte);
				...
				Log.e("zyh","startByte-----> "+o.hashCode()+"-->true 2 ");
				...
				iBeacon.txPower = -55;
				iBeacon.distance = String.format("%.2f", calculateAccuracy(iBeacon.txPower, rssi));
				Log.e("zyh","startByte-----> "+o.hashCode()+"-->true 3 ");
				return iBeacon;
			}
			Log.e("zyh","startByte---++++-----> "+o.hashCode()+"-->  "+startByte);
			startByte++;
		}
		if (patternFound == false) {
			// 这不是iBeacon
			Log.e("zyh","startByte----> "+o.hashCode()+"-->  no ");

			return null;
		}
		Log.e("zyh","when crate ibeacon startByte--> "+startByte);
		iBeacon iBeacon = new iBeacon();
